...
- The main agent owns planning, edits, verification, and final synthesis. Read-only subagents provide bounded evidence.
- Use `explorer` first when the task requires code search, flow tracing, symbol discovery, pattern mapping, or multi-file ownership discovery. Give it all anchors in one request and reuse its Evidence Packet.
- Use the read-only impact role (`repo_impact` in Codex; `repo-impact` in Claude Code, Copilot, and Antigravity) when a change can cross API, shared-type, database, importer, retention, or UI boundaries.
- Use at most one explorer and one impact review per task, depth 1. Do not delegate implementation unless the user explicitly requests implementation workers.
