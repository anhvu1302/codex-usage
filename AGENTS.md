...
- The main agent owns planning, routing, write-ownership assignment, final synthesis, integration, and task-level verification. It may keep direct edits or delegate only already-decided, non-overlapping implementation partitions.
...
- After behavior and interfaces are decided, the main agent may automatically fan out two or three `repo_worker` instances only for independent, substantially similar edits across disjoint exact file sets. Before spawning workers, create an explicit ownership map including the parent and assign exactly one writer to every path.
- Each worker prompt must use `fork_turns="none"` and state the owning repository root, exact allowed write paths, approved behavior and local reference pattern, narrow verification command or reason none is needed, and explicit confirmation that no other writer owns those paths. Workers share the filesystem, must preserve concurrent changes, and must stop on a conflict or boundary expansion.
- Do not fan out unresolved design or contract decisions, dependent edit sequences, migrations, generated/codegen work, shared intermediate output, or shared central files. The main agent reviews every worker diff, resolves integration issues, and runs final task-level verification.
