...
CODEX_HOOK_PAYLOAD="$payload_text" python3 - "$@" <<'PY'
import json
import os
import re
import shlex
...
payload_text = os.environ.get("CODEX_HOOK_PAYLOAD", "")
try:
...
def add_notice(reason: str, message: str):
    notices.append(f"{reason}: {message}")


def confirmation_notice(reason: str):
    add_notice(
        "Explicit user authorization required",
        f"{reason} Continue only when the current user explicitly requested this action; otherwise stop and ask.",
    )
...
def emit_notices():
    ...
    warn("Codex Usage policy notice", " ".join(unique))
...
for path in resolved_paths:
    ...
    if write_tool and is_dependency_manifest(path):
        confirmation_notice("Dependency manifest and lockfile changes require confirmation.")
    if write_tool and not is_within(path, [workspace_root]):
        confirmation_notice("File writes outside the active repository require confirmation.")
...
if is_shell_tool:
    if not command_text:
        confirmation_notice("The terminal command could not be determined from the hook payload.")
        emit_notices()
    try:
        ...
        segments = None
    if not segments:
        confirmation_notice("Opaque shell syntax, substitution, redirection, backgrounding, or embedded newlines require confirmation.")
        emit_notices()
    ...
    if not all(safe_command(segment) for segment in segments):
        confirmation_notice("This terminal command is outside the Codex Usage safe allowlist.")
    emit_notices()
    raise SystemExit(0)
